...
- 2026-09-05 (post-hoc, `/code-review`) — Found and fixed a real gap this spec introduced: `completeOrder()`/`uncompleteOrder()`/`cancelOrder()` read-checked-wrote `status` with no transaction or row lock, so two concurrent requests on the same order could both read a pre-transition status and race — a complete landing after a cancel could silently overwrite it back to `done`, defeating the "cancelled is terminal" guarantee this same spec added. Fixed by wrapping each in `DB::transaction()` with `Order::where('id', $id)->lockForUpdate()->firstOrFail()`, matching the locking discipline spec 019 already applied to order_number allocation in the same file. Verified live: 8 concurrent cancel + 8 concurrent complete requests against one order — some completes legitimately won before the cancel serialized in (valid, since nothing had cancelled yet), but once cancel landed, every subsequent request was correctly rejected and the order's final status stayed `cancelled`. Also fixed `OrderService::removeOrderItem()`'s error message, which still said "Exclua o pedido inteiro" (delete the whole order) after this same spec removed that capability — now says "Cancele o pedido inteiro."
- 2026-09-05 (post-hoc, second `/code-review` pass) — Found a second, adjacent gap: only the three status-*transition* methods above treated `cancelled` as terminal — `updateOrder()`, `addOrderItem()`, `updateOrderItem()`, and `removeOrderItem()` never checked the parent order's status at all, so a cancelled order's fields and items could still be silently mutated, undermining the exact audit-preservation guarantee this spec introduced cancellation for. Added the same guard to all four, via a new small typed `App\OrderCancelledException` (not a plain `\DomainException`) so `OrderController` can distinguish it from the other, differently-coded `\DomainException` cases already caught in `addItem()`/`removeItem()` (menu-item-unavailable, last-item-cannot-be-removed) without message-sniffing. Verified via 4 new `OrderRepositoryTest` cases, one per guarded method.
- 2026-09-05 (post-hoc, second `/code-review` pass) — Also fixed `public/kitchen/app.js`'s SSE listener: it still listened for `order.deleted`, an event this spec's own implementation removed in favor of `order.cancelled` — but the listener itself was never updated, so a cancellation on one open kitchen screen never refreshed another. Replaced the dead listener with the correct one.
