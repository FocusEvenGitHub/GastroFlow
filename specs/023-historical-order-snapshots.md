...
- 2026-09-05 (post-hoc, `/code-review`) — Found and fixed a real bug in `getTopItems()`'s display-name choice: `MAX(order_items.item_name)` is an alphabetical maximum, not "most recent" as Functional requirement 6 actually specifies — this spec's own live validation (Acceptance criterion 5) only happened to pass because its test rename ("Beterraba" → "Beterraba Renomeada") sorted later alphabetically too, masking the bug. Replaced with `SUBSTRING_INDEX(GROUP_CONCAT(item_name ORDER BY id DESC SEPARATOR '\u{7}'), '\u{7}', 1)` — takes the name from the highest-`id` (most recent) row per `menu_item_id`, safe from `group_concat_max_len` truncation since MySQL truncates from the end and the wanted entry is always first. Re-verified live with a rename in the direction that would have exposed the original bug ("Beterraba" → "Aterraba", alphabetically *earlier*): the report correctly returned "Aterraba", which the old code would have failed (it would have kept returning "Beterraba").
