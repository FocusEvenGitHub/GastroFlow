...
- 2026-09-05 (post-hoc, second `/code-review` pass) — Found and fixed a real gap in `validateOrderData()`'s own custom `notes` check: `isset($item['notes']) && is_string($item['notes']) && mb_strlen(...) > MAX` used `is_string()` as a *prerequisite* to even attempt the length check, so a non-string `notes` value (array/number/bool) skipped the check entirely and passed validation, then broke deep inside `OrderRepository::createOrder()`'s transaction with an uncaught error instead of a clean `400`. Fixed by rejecting a non-string value outright. (Confirmed, while investigating, that Valitron's own built-in `lengthMax` rule — used by `validateOrderItemAdd()`/`validateOrderItemUpdate()` — already fails safe on a non-string value via its internal `stringLength()` helper, so this gap was specific to this one custom closure, not a wider pattern.) Added `testNonStringNotesIsRejectedOnOrderCreation`.
