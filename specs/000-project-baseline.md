...
`PrintService` builds an ESC/POS receipt (header, items, packaging labels, total, footer) via `mike42/escpos-php` over `NetworkPrintConnector`, plus a `printTestPage()` diagnostic used by `POST /api/admin/settings/test-print`. Printing is dispatched asynchronously through a custom DB-backed job queue: `src/Jobs/PrintOrderJob.php` + `src/Services/JobService.php` + `src/Models/Job.php` (table `jobs`, migrations `common/migrations/007_jobs.sql` and `016_job_reliability.sql`), processed by the long-running `bin/worker` CLI script. **Corrected 2026-09-25 (spec 042's doc pass)**: at the time this baseline was written, print failures were "best effort" — logged but never thrown. Spec 038 changed this: `printOrder()` now rethrows on failure so the job queue can retry it and the order is never silently recorded as printed; print (and job-failure) logs also now carry structured `order_id`/`job_id`/`event`/`request_id` context via Monolog, not just a message string (spec 042).
