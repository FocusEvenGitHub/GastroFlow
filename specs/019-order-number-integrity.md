...
- 2026-09-05 (post-hoc, second `/code-review` pass over the whole branch) — Found and fixed a real, reproduced-live bug in `allocateNextNumber()`: a manual `order_number` that happens to equal `counter+1` made every subsequent auto-assign recompute that exact same doomed value forever, because the whole transaction — including the counter's own increment — rolled back on the resulting collision. Reproduced against the real dev DB: created a manual order at the next auto value, then two consecutive auto-assign attempts both collided identically, and the counter never advanced. Fixed by checking, still under the same row lock, whether the candidate number is already taken for that `business_date` and skipping forward until a free one is found; re-verified live immediately after — the next request correctly skipped the taken number. Also batched `createOrder()`'s per-item `MenuItem` lock into one `whereIn()->lockForUpdate()`, sorted by id — the same review pass flagged the original per-item, client-order-dependent locking as a real lock-ordering deadlock risk between two concurrent orders referencing the same items in opposite sequence; verified live with 12 concurrent orders using the exact adversarial reversed-item-order pattern, no failures.
