...
**Known gap, flagged 2026-09-26 (external review), not yet fixed:** `SettingsController::updateSettings()` (`src/Controllers/SettingsController.php`) records every changed setting key's `old`/`new` value verbatim into `audit_log`, with no redaction or allowlist — confirmed in code, no filtering exists today. This is fine for the current field set (restaurant name, printer IP, etc.), but `v2.1.0`'s fiscal settings (certificate password, CSC — see `docs/ROADMAP.md`'s "Fiscal settings + secure A1 certificate handling") are explicitly write-only secrets that must **never** be logged anywhere, per that same milestone's own stated security rules. If fiscal settings are added to the generic `settings` table without also updating `SettingsController::updateSettings()`'s audit-log call, a certificate password or CSC would land permanently in `audit_log` the first time someone changes it. This needs a sensitive-field redaction rule (`[REDACTED]`) or an explicit allowlist of auditable keys, put in place *before* fiscal settings exist, not discovered after. Tracked in Trello, linked from the "[v2.1] Fiscal settings + secure A1 certificate" card.
...
- CORS defaults to `*` when `CORS_ALLOWED_ORIGIN` is unset; configurable per spec 001, but the permissive default is still an open gap (`docs/ROADMAP.md`'s `v1.6.0 — Baseline & Security` phase doesn't yet name a fix for the default itself). **Consciously accepted as v1.8 debt** (Community assumes a trusted LAN, so this isn't a milestone blocker) — **but flagged for `v2.0`'s release-candidate hardening pass**: `APP_ENV=production` should not silently fall back to `*` the way it does today. Tracked in Trello ("CORS: default permissivo `*`").
